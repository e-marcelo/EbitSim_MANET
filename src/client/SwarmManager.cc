...
//EAM::            }
    ...
    void socketClosed(int connId, void *yourPtr) {
        ...
        //[EAM] this->socket->setDataTransferMode(TCPDataTransferMode::TCP_TRANSFER_OBJECT);
    ...
    void sendAnnounce(AnnounceType type) {
        ...
        if (this->socket->getState() == TCPSocket::NOT_BOUND) {
            //EAM **
            //[EAM] this->socket->setDataTransferMode(TCP_TRANSFER_OBJECT);
            //EAM **
...
void SwarmManager::initialize() {
    ...
    //EAM :: port
    this->localPort = bitTorrentClient->par("localPort").longValue();
